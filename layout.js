...
function BinaryCircle(node1,node2,priority) {
	...
	function calcGapMiddle(c1,c2) {
		...
		return {x:gapCenterX,y:gapCenterY};
	};
...
function CirclePeaks(node){
	...
	this.peak_left={x:node.x-node.r,y:node.y};
	this.peak_right={x:node.x+node.r,y:node.y};
	this.peak_bottom={x:node.x,y:node.y+node.r};
	this.peak_bottom_left={x:(node.x-(node.r*.707)),y:(node.y+(node.r*.707))};
	this.peak_bottom_right={x:(node.x+(node.r*.707)),y:(node.y+(node.r*.707))};
	this.peak_top={x:node.x,y:node.y-node.r};
	this.peak_top_left={x:(node.x-(node.r*.707)),y:(node.y-(node.r*.707))};
	this.peak_top_right={x:(node.x+(node.r*.707)),y:(node.y-(node.r*.707))};
...
function sortBinaryCircles(node){
   node.binaries = node.binaries.sort(function (a,b) {return d3.descending(a.gap, b.gap);});
}
...
this.apply=function(){
   ...
   nodes = nodes.sort(function (a,b) {return d3.descending(a.r, b.r);});
...
function processNode(node){
	...
	var subNodes=node.children.sort(function (a,b) {return d3.descending(a.r, b.r);});;
	
    var a=subNodes[0],b=subNodes[1],c=subNodes[2],d=subNodes[3];
	...
	if(subNodes.length>13){
	    ...
	    var e=subNodes[4],f=subNodes[5],g=subNodes[6],h=subNodes[7];
